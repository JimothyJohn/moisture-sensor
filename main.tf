...
locals {
  ...
  thing_name = aws_iot_thing.moisture_sensor.name
  region     = "us-east-1"
}

data "aws_iam_policy_document" "moisture_sensor_policy_doc" {
  statement {
    effect    = "Allow"
    actions   = ["iot:*"]
    resources = ["*", ]
...
resource "aws_iot_policy" "moisture_sensor_policy" {
  name   = "moisture_sensor_policy"
  policy = data.aws_iam_policy_document.moisture_sensor_policy_doc.json
}

resource "aws_iot_policy_attachment" "att" {
  policy = aws_iot_policy.moisture_sensor_policy.name
